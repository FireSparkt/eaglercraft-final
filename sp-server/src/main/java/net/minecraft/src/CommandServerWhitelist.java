	...
	public void processCommand(ICommandSender par1ICommandSender, String[] par2ArrayOfStr) {
		if (par2ArrayOfStr.length >= 1) {
			...
			if (par2ArrayOfStr[0].equals("list")) {
				par1ICommandSender.sendChatToPlayer(par1ICommandSender.translateString("commands.whitelist.list",
						new Object[] {
				...
				par1ICommandSender.sendChatToPlayer(joinNiceString(MinecraftServer.getServer().getConfigurationManager()
						.getWhiteListedPlayers().toArray(new String[0])));
				return;
			}
			...
			if (par2ArrayOfStr[0].equals("remove")) {
				...
				notifyAdmins(par1ICommandSender, "commands.whitelist.remove.success",
			...
			if (par2ArrayOfStr.length == 2) {
				if (par2ArrayOfStr[0].equals("add")) {
					...
					for (int var8 = 0; var8 < var7; ++var8) {
						...
						if (doesStringStartWith(var5, var9) && !MinecraftServer.getServer().getConfigurationManager()
				...
				if (par2ArrayOfStr[0].equals("remove")) {
					return getListOfStringsFromIterableMatchingLastWord(par2ArrayOfStr,
							MinecraftServer.getServer().getConfigurationManager().getWhiteListedPlayers());
				}
			}
