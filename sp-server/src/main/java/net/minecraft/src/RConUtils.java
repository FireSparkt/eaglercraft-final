...
public class RConUtils {
	...
	/**
	 * Read 4 bytes from the given array in little-endian format and return them as
	 * an int
	 */
	public static int getBytesAsLEInt(byte[] par0ArrayOfByte, int par1, int par2) {
		return 0 > par2 - par1 - 4 ? 0
				: par0ArrayOfByte[par1 + 3] << 24 | (par0ArrayOfByte[par1 + 2] & 255) << 16
						| (par0ArrayOfByte[par1 + 1] & 255) << 8 | par0ArrayOfByte[par1] & 255;
	}

	/**
	 * Read 4 bytes from the given array in big-endian format and return them as an
	 * int
	 */
	public static int getBytesAsBEint(byte[] par0ArrayOfByte, int par1, int par2) {
		return 0 > par2 - par1 - 4 ? 0
				: par0ArrayOfByte[par1] << 24 | (par0ArrayOfByte[par1 + 1] & 255) << 16
						| (par0ArrayOfByte[par1 + 2] & 255) << 8 | par0ArrayOfByte[par1 + 3] & 255;
	}
